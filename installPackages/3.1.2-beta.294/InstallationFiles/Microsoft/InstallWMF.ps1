...
{
    $packageName = 'W2K12-KB3191565-x64.msu'
}
...
if($packagePath -and (Test-Path $packagePath))
{
        if(Test-Compatibility)
        {
            ...
            if($PSCmdlet.ShouldProcess($packagePath,"Install WMF 5.1 Package from:"))
            {
                ...
                $output = Start-Process -FilePath $wusaExe -ArgumentList $wusaParameterString -Wait -PassThru 
				if(($output.ExitCode -ne 0) -and ($output.ExitCode -ne 3010) -and ($output.ExitCode -ne 2359302))
				{
					$errorParameters = @{
                                    exception = 'System.Exception';
                                    exceptionMessage = "WMF 5.1 did not install correctly. Please run Setup again. Exit code:{0}" -f $output.ExitCode;
